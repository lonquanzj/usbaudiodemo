...
wxCopyFile (const wxString& file1, const wxString& file2, bool overwrite)
{
    ...
    if ( wxStat( file1.c_str(), &fbuf) != 0 )
    {
        ...
        // from it anyhow
        //wxLogSysError(_("Impossible to get permissions for file '%s'"), file1.c_str());
        return false;
    }
    ...
    {
        //wxLogSysError(_("Impossible to overwrite the file '%s'"), file2.c_str());
        return false;
    }
    ...
    {
        //wxLogSysError(_("Impossible to set permissions for the file '%s'"),              file2.c_str());
        return false;
    }
#endif // OS/2 || Mac
...
bool wxFindFileInPath(wxString *pStr, const wxChar *pszPath, const wxChar *pszFile)
{
    ...
    wxString strFile;
    wxChar *pc, *save_ptr;
    for ( pc = strtok_r(szPath, wxPATH_SEP, &save_ptr);
          pc != NULL;
          pc = strtok_r((wxChar *) NULL, wxPATH_SEP, &save_ptr) )
    {
